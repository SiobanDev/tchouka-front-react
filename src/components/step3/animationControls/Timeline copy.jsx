...
const TimeLine = ({allImageDelayList}) => {
  ...
  const [landmarkStyle, setLandmarkStyle] = useState({
    left: 0,
  });
  const [movingCursor, setMovingCursor] = useState(false);
  const [mousePositionX, setMousePositionX] = useState(0);
  const [containerWidth, setContainerWidth] = useState(0);
  const landmarkPositionList = allImageDelayList.map((imageDelay, i, allImageDelayList) => {return Math.floor(imageDelay * 100 / allImageDelayList[allImageDelayList.length - 1])})
